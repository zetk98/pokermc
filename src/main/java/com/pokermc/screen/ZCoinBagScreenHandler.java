...
public class ZCoinBagScreenHandler extends Generic3x3ContainerScreenHandler {
    ...
    public void onClosed(PlayerEntity player) {
        ...
        if (!player.getWorld().isClient && bagSlotIndex >= 0 && bagSlotIndex <= 40) {
    ...
    public static void loadFromBag(ItemStack bagStack, Inventory inv) {
        if (bagStack.isEmpty() || !(bagStack.getItem() instanceof com.pokermc.item.ZCoinBagItem)) return;
        ...
        if (container != null) {
            ...
            int balance = com.pokermc.item.ZCoinBagItem.getBalance(bagStack);
            if (balance > 0) {
                inv.clear();
    ...
    public static void saveToBag(ItemStack bagStack, Inventory inv) {
        ...
        bagStack.remove(com.pokermc.component.PokerComponents.ZCOIN_BAG_BALANCE);
    }
}
