...
public class BlackjackTableScreen extends Screen {
    ...
    public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
        ...
        ctx.drawTexture(RenderPipelines.GUI_TEXTURED, TEX_BG, tableTx, tableTy, 0, 0, tableW, tableH, 360, 230);
        ...
        for (int i = 0; i < dealerHand.size(); i++) {
            ...
            if (deal != null) {
                ...
                if (prog >= 1f) {
                    ...
                    ctx.drawTexture(RenderPipelines.GUI_TEXTURED, TEX_CARD_BACK, (int) drawX, (int) drawY, 0, 0, cardW, cardH, cardW, cardH);
                    continue;
                }
            }
            ...
            if ("??".equals(code) || !show) {
                ctx.drawTexture(RenderPipelines.GUI_TEXTURED, TEX_CARD_BACK, x, y, 0, 0, cardW, cardH, cardW, cardH);
    ...
    private void drawCard(DrawContext ctx, String code, int x, int y, int w, int h) {
        if ("??".equals(code)) {
            ctx.drawTexture(RenderPipelines.GUI_TEXTURED, TEX_CARD_BACK, x, y, 0, 0, w, h, w, h);
            return;
        }
        try {
            Card c = Card.fromCode(code);
            int u = (c.rank().ordinal() % 13) * 22;
            int v = c.suit().ordinal() * 32;
            ctx.drawTexture(RenderPipelines.GUI_TEXTURED, TEX_CARD_ATLAS, x, y, u, v, w, h, ATLAS_W, ATLAS_H);
        } catch (Exception e) {
            ctx.drawTexture(RenderPipelines.GUI_TEXTURED, TEX_CARD_BACK, x, y, 0, 0, w, h, w, h);
        }
    }
            ...
            if (isHero) {
                ...
                if (deal != null) {
                    ...
                    if (prog >= 1f) {
                        ...
                        float drawX = CardAnimationHelper.lerpEased(deal.fromX(), deal.toX(), t, true);
                        float drawY = CardAnimationHelper.lerpEased(deal.fromY(), deal.toY(), t, true);
                        ctx.drawTexture(RenderPipelines.GUI_TEXTURED, TEX_CARD_BACK, (int) drawX, (int) drawY, 0, 0, SMALL_W, SMALL_H, CARD_W, CARD_H, CARD_W, CARD_H);
                        continue;
                    }
                }
                ...
                if (!show) {
                    ctx.drawTexture(RenderPipelines.GUI_TEXTURED, TEX_CARD_BACK, ox, cardY, 0, 0, SMALL_W, SMALL_H, SMALL_W, SMALL_H);
                } else if (flipStart != null) {
                    float prog = CardAnimationHelper.getFlipProgress(flipStart);
                    if (prog >= 1f) {
