...
public final class CardAnimationHelper {
    ...
    public static void drawCardWithFlip(DrawContext ctx, int x, int y, int w, int h,
        ...
        ms.pushMatrix();
        ms.translate(x + w / 2f, y + h / 2f);
        ms.scale(scaleX, 1f);
        ms.translate(-w * scaleX / 2f, -h / 2f);

        if (eased < 0.5f) {
            ctx.drawTexture(RenderPipelines.GUI_TEXTURED, texBack, 0, 0, 0, 0, w, h, regionW, regionH, regionW, regionH);
        } else {
            ctx.drawTexture(RenderPipelines.GUI_TEXTURED, texAtlas, 0, 0, (float) atlasU, (float) atlasV, w, h, regionW, regionH, atlasW, atlasH);
        }
        ms.popMatrix();
    }
    ...
    public static void drawCardAt(DrawContext ctx, float x, float y, int w, int h,
        ...
        ctx.drawTexture(RenderPipelines.GUI_TEXTURED, tex, (int) x, (int) y, u, v, w, h, texW, texH);
    }

    public static void drawCardBackAt(DrawContext ctx, float x, float y, int w, int h, Identifier texBack) {
        ctx.drawTexture(RenderPipelines.GUI_TEXTURED, texBack, (int) x, (int) y, 0, 0, w, h, w, h);
    }
